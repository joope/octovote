OctoVote.controller('MovieController', function($scope, $location, $timeout, currentAuth, FirebaseService){
        ...
        if(!$scope.userData.posted){
            if($scope.newMovie !== '' && $scope.newNick !== ''){
                FirebaseService.addMovie({
                    ...
                    userID: $scope.userLoggedIn.uid,
                    link: $scope.movieLink,
                    votes: 0
                });
                FirebaseService.addUser($scope.userLoggedIn.uid,{
                    posted: true,
                    votes: 5
                });
                $location.path('/');
